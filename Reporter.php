...
class Reporter
{
	...
	/**
     * Contains if JEXEC security is found.
     *
     * @var boolean
     */
    private $jexec_found = false;
    ...
    public function prepareFileReport(File $phpcsFile)
    {
        ...
        foreach ($phpcsFile->getErrors() as $line => $lineErrors) {
            foreach ($lineErrors as $column => $colErrors) {
                ...
                foreach ($colErrors as $data) {					
					if ($data['message'] == "JEXEC found")
					{
						if (!$this->jexec_found)
						{
							$this->jexec_found = true;							
						}
						continue;
		...
		// Look for the 'JEXEC' check
		if (!$this->jexec_found)
		{
			$newErrors[] = [
                'message'  => "The JEXEC security check was not found in this file.",
                'source'   => "Joomla.Rules.Jexec.Not Found",
				'severity' => "",
                'fixable'  => "",
                'type'     => 'ERROR',
            ];
